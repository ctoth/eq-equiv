...
def equation_signature(equation: BoundEquation) -> tuple[str, tuple[str, ...]] | None:
    """Build a concept-based grouping signature for a typed equation."""
    concept_ids = sorted(
        {
            variable.concept_id
...
def canonicalize_equation(
    ...
    """Normalize a typed equation onto a deterministic canonical form."""
    bindings = _equation_bindings(equation)
    if isinstance(bindings, EquationFailure):
        return bindings

    source_text = _equation_source_text(equation)
...
def structural_signature(equation: BoundEquation) -> str | EquationFailure:
    normalization = canonicalize_equation(equation)
...
def compare_equations(
    equation_a: BoundEquation,
    equation_b: BoundEquation,
    *,
    domain_assumptions: Sequence[DomainAssumption] = (),
) -> EquationComparison:
    assumptions = tuple(domain_assumptions)
    left = _canonicalize_equation(equation_a, assumptions)
    right = _canonicalize_equation(equation_b, assumptions)
